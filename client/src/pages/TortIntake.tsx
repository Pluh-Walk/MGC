...
import {
  Scale, ArrowLeft, AlertTriangle, AlertCircle, CheckCircle2,
...
const TORT_CASE_TYPES = [
  { value: 'vehicular_accident',   label: 'Vehicular Accident',                          defaultSubject: 'Claim for damages — vehicular accident' },
  { value: 'medical_negligence',   label: 'Medical Negligence / Malpractice',             defaultSubject: 'Claim for damages — medical negligence / malpractice' },
  { value: 'personal_injury',      label: 'Personal Injury',                              defaultSubject: 'Claim for damages — personal injury' },
  { value: 'property_damage',      label: 'Damage to Property Through Negligence',        defaultSubject: 'Claim for damages — negligent damage to property' },
  { value: 'defamation',           label: 'Defamation (Libel / Slander)',                 defaultSubject: 'Action for damages — defamation' },
  { value: 'product_liability',    label: 'Product Liability',                            defaultSubject: 'Claim for damages — defective product / product liability' },
  { value: 'premises_liability',   label: 'Premises Liability',                           defaultSubject: 'Claim for damages — unsafe premises' },
  { value: 'employer_liability',   label: 'Employer Liability / Vicarious Liability',     defaultSubject: 'Claim for damages — employer / vicarious liability' },
  { value: 'nuisance',             label: 'Nuisance',                                     defaultSubject: 'Action for abatement of nuisance and damages' },
  { value: 'other',                label: 'Other Quasi-Delict / Tort',                    defaultSubject: '' },
]

const MAX_FILES = 10
const MAX_MB    = 20

export default function TortIntake() {
  useAuth()
  ...
  const handleTortTypeChange = (value: string) => {
    const found = TORT_CASE_TYPES.find(t => t.value === value)
    setForm(prev => ({
      ...prev,
      tort_case_type: value,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.tort_case_type)   { setError('Please select the type of tort / quasi-delict case.'); return }
    ...
    try {
      ...
      fd.append('tort_case_type',  form.tort_case_type)
        ...
        <div style={{ marginTop: '1rem', maxWidth: 760, margin: '1rem auto 0' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              <h1 style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--text)' }}>
                Tort / Quasi-Delict Complaint Intake
              </h1>
            </div>
            <p style={{ color: 'var(--text-muted)', lineHeight: 1.7, fontSize: '0.9rem' }}>
              Use this form to describe your claim arising from negligence, fault, or a wrongful act
              that caused you injury or damage. An attorney will review your submission and respond.
              Please be as specific as possible about the incident and the damages you suffered.
            </p>
            <div style={{
              ...
              <strong>Note:</strong> Under Art. 2176 of the Civil Code, quasi-delicts cover acts or omissions causing
              damage through fault or negligence. If both parties reside in the same barangay,
              barangay conciliation may still be required before filing in court.
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{
              ...
              {/* Tort Case Type + Subject (2-col) */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1rem', marginBottom: '1.25rem' }}>
                <div>
                  <label style={labelStyle}>Type of Tort Case <span style={{ color: 'var(--danger)' }}>*</span></label>
                  <div style={{ position: 'relative' }}>
                    <select
                      value={form.tort_case_type}
                      onChange={e => handleTortTypeChange(e.target.value)}
                      style={{ ...inputStyle, appearance: 'none', paddingRight: '2rem' }}
                    >
                      <option value="">Select tort case type…</option>
                      {TORT_CASE_TYPES.map(t => (
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="Brief title of your complaint (e.g. Vehicular accident causing bodily injury — March 5, 2026)"
                    value={form.subject}
              ...
              <div style={fieldStyle}>
                ...
                <textarea
                  rows={6}
                  placeholder="Describe what happened in as much detail as possible. Include when and where the incident occurred, how the other party was negligent or at fault, and what injuries or damages you suffered as a result."
                  value={form.narration}
                  onChange={e => set('narration', e.target.value)}
                  style={{ ...inputStyle, resize: 'vertical', fontFamily: 'inherit' }}
                />
                <p style={hintStyle}>Clearly describe the negligent act and the resulting damage or injury.</p>
              </div>
              ...
              <div style={fieldStyle}>
                ...
                <textarea
                  rows={3}
                  placeholder="Which law or provision do you believe was violated? (e.g. Art. 2176 NCC — quasi-delict, Art. 2180 — vicarious liability, Art. 2219 — moral damages, RA 4136 Land Transportation Code, etc.)"
              ...
              <div style={fieldStyle}>
                ...
                <textarea
                  rows={3}
                  placeholder="What do you want the court to order? (e.g. actual damages ₱500,000 for medical expenses, loss of earning capacity, moral damages, exemplary damages, attorney's fees, etc.)"
              ...
              {/* Responsible Party + Incident Date */}
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem', marginBottom: '1.25rem' }}>
                <div>
                  <label style={labelStyle}>Responsible Party</label>
                  <input
                    type="text"
                    placeholder="Full name or company name of the person/entity at fault"
              ...
              <div style={{
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '1rem' }}>
                  If both parties reside in the same city or barangay, barangay conciliation may be required before filing
                  in court. Upload a photo of your Certificate to File Action issued by the Lupong Tagapamayapa if you have one.
                </p>
                <div
              ...
              <div style={fieldStyle}>
                ...
                >
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                    Click or drag files here — medical records, photos, police reports, affidavits, receipts, etc.
                  </p>
