...
import {
  Scale, FileText, Clock, MessageSquare, Megaphone,
  Search, Plus, Send, Paperclip, X, Loader2, Image, MoreHorizontal,
  Trash2, MapPin, ChevronRight, Briefcase, Calendar, Users, User,
} from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import SettingsDropdown from '../components/SettingsDropdown'
import NotificationBell from '../components/NotificationBell'
import UserAvatar from '../components/UserAvatar'
import { hearingsApi, announcementsApi, messagesApi, secretaryApi } from '../services/api'
...
export default function SecretaryDashboard() {
  ...
  useEffect(() => {
    secretaryApi.getAttorneyInfo().then(r => setAttorney(r.data.data)).catch(() => {})
  }, [])
  ...
  const loadThread = useCallback(async (pid: number) => {
    ...
    try { const r = await messagesApi.thread(pid); setThread(r.data.data) }
    finally { setLoadingThread(false) }
  }, [])

  useEffect(() => {
    if (!activeId) { if (pollRef.current) clearInterval(pollRef.current); return }
    loadThread(activeId)
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest('.msg-actions-wrap')) setMenuId(null)
    }
      ...
      <div className="dash-body">
        ...
        <aside className="dash-sidebar">
          ...
          {attorney && (
            <div className="sidebar-attorney-card">
              <div className="sidebar-atty-header">
                <div className="sidebar-atty-avatar">
                  {attorney.fullname?.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="sidebar-atty-name">{attorney.fullname}</div>
                  <div className="sidebar-atty-role">Your Attorney</div>
                </div>
                <span
                  className="avail-dot"
                  style={{ background: attorney.availability === 'available' ? '#22c55e' : attorney.availability === 'busy' ? '#ef4444' : '#f59e0b' }}
                  title={attorney.availability ?? 'unknown'}
                />
              </div>
              {attorney.email && (
                <a href={`mailto:${attorney.email}`} className="sidebar-atty-contact">
                  <User size={11} /> {attorney.email}
                </a>
              )}
              {attorney.phone && (
                <div className="sidebar-atty-contact">
                  <MapPin size={11} /> {attorney.phone}
                </div>
              )}
            </div>
          )}
        ...
        <main className="dash-center">
          ...
          <div className="sec-stats-row">
            <div className="sec-stat-card" onClick={() => navigate('/cases')} style={{ cursor: 'pointer' }}>
              <Briefcase size={20} className="sec-stat-icon" />
              <div>
                <div className="sec-stat-value">{hearings.length > 0 || !loadingH ? hearings.filter(h => h.status === 'scheduled').length : '—'}</div>
                <div className="sec-stat-label">Scheduled Hearings</div>
              </div>
            </div>
            <div className="sec-stat-card" onClick={() => navigate('/hearings')} style={{ cursor: 'pointer' }}>
              <Calendar size={20} className="sec-stat-icon" />
              <div>
                <div className="sec-stat-value">{upcomingCount}</div>
                <div className="sec-stat-label">Upcoming</div>
              </div>
            </div>
            <div className="sec-stat-card">
              <MessageSquare size={20} className="sec-stat-icon" />
              <div>
                <div className="sec-stat-value">{totalUnread > 0 ? totalUnread : '0'}</div>
                <div className="sec-stat-label">Unread Messages</div>
              </div>
            </div>
            <div className="sec-stat-card" onClick={() => navigate('/announcements')} style={{ cursor: 'pointer' }}>
              <Megaphone size={20} className="sec-stat-icon" />
              <div>
                <div className="sec-stat-value">{loadingA ? '—' : announcements.length}</div>
                <div className="sec-stat-label">Announcements</div>
              </div>
            </div>
          </div>
